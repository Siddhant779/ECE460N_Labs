...
int GetLD_PTBR(int *x)       { return(x[LD_PTBR]); }          
...
int GetGATE_ADDER_VA(int *x) { return(x[GATE_ADDER_VA]); }
int GetGATE_VA(int *x)       { return(x[GATE_VA]); }
int GetGATE_ADDER_PTE(int *x){ return(x[GATE_ADDER_PTE]); }
...
int value_gatePTBR=0;
int value_gateADDER_VA=0;
int value_gateVA=0;
int value_gateADDER_PTE=0;
int value_gatePTE=0;
int value_muxPTE=0;
...
void eval_micro_sequencer() {
  ...
  if(first_cycle==0){
    ...
    if(GetRS(CURRENT_LATCHES.MICROINSTRUCTION)==1){
        return_state = GetJ(CURRENT_LATCHES.MICROINSTRUCTION);
    }
...
void eval_bus_drivers() {
  ...
  int spMux = GetSPMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    ...
    int gateMDRStatus = GetGATE_MDR(CURRENT_LATCHES.MICROINSTRUCTION);
    if(gateMDRStatus==1){
    if(data_size==0){
    ...
    //for lab 5 
    int ptemux = GetPTEMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int tramux = GetTRAMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    value_gatePTBR = CURRENT_LATCHES.PTBR;
    value_gateVA = CURRENT_LATCHES.VA;
    int temp[CONTROL_STORE_BITS];
    for(int i=0;i<CONTROL_STORE_BITS;i++){
         temp[i] = CONTROL_STORE[return_state][i];
     }
    int rw = GetR_W(temp);
    rw=rw<<1;
    rw+=1;
    if(ptemux==1){
        value_muxPTE = CURRENT_LATCHES.PTE | rw;
    }
    value_gatePTE = value_muxPTE;
    value_gateADDER_PTE = (((INSTR(9,5,CURRENT_LATCHES.PTE))<<9) +(INSTR(0,9,CURRENT_LATCHES.VA))) & 0x3FFF;
    value_gateADDER_VA = (((INSTR(9,7,CURRENT_LATCHES.VA))<<1)+ (INSTR(8,8,CURRENT_LATCHES.PTBR)<<8));
...
void drive_bus() {
   ...
   */       
    ...
    int GateAdderva = GetGATE_ADDER_VA(CURRENT_LATCHES.MICROINSTRUCTION);
    int Gateva = GetGATE_VA(CURRENT_LATCHES.MICROINSTRUCTION);
    int GateAdderpte = GetGATE_ADDER_PTE(CURRENT_LATCHES.MICROINSTRUCTION);
    ...
    if(Gatepc || Gatemdr || Gatealu || GateMarmux || Gateshf || GateSP || GatePSR || GateTable || Gateptbr || GateAdderva || Gateva || GateAdderpte || Gatepte) {
        if(Gateptbr){
            BUS= Low16bits(value_gatePTBR);
        }
        if(GateAdderva){
            BUS= Low16bits(value_gateADDER_VA);
        }
        if(Gateva){
            BUS= Low16bits(value_gateVA);
        }
        if(GateAdderpte){
            BUS= Low16bits(value_gateADDER_PTE);
        }
        if(Gatepte){
            BUS= Low16bits(value_gatePTE);
        }
        if(GateTable){
...
void latch_datapath_values() {
   ...
   */       
  int PSR_changed = 0;
    int ld_pte = GetLD_PTE(CURRENT_LATCHES.MICROINSTRUCTION);
    int ld_va = GetLD_VA(CURRENT_LATCHES.MICROINSTRUCTION);
    int mux_pte = GetPTEMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int ld_ptbr = GetLD_PTBR(CURRENT_LATCHES.MICROINSTRUCTION);
    ...
    if(CURRENT_LATCHES.STATE_NUMBER==8 && exception_occur==1){
        exception_occur=0;
    }
    int interr  = inter_request && CURRENT_LATCHES.STATE_NUMBER==18 && !exception_occur;
    ...
    if(IRD==2){
        int rs = GetRS(CURRENT_LATCHES.MICROINSTRUCTION);
        if(rs==1){
            ...
            int protectBit = INSTR(3,1,NEXT_LATCHES.PTE);
            int valid = INSTR(2,1,NEXT_LATCHES.PTE);
            int value = (protectBit==0 && psr15==1 && exception_caused==0) || (protectBit==0 && return_state!=28 && exception_caused==0);
            if(protectBit==0 && !(psr15==0 || return_state==28) && exception_caused==0){
                exception_caused=1;
                NEXT_LATCHES.VECT=protect;
            }
            if(exception_caused==0 && valid==0){
                printf("page fault here");
                exception_caused=1;
                NEXT_LATCHES.VECT=pageFault;
            }
        ...
        if(exception_caused==0 && interr==0 && rs==1){
            for(int i=0;i<CONTROL_STORE_BITS;i++){
                next_instr[i] = CONTROL_STORE[34][i];
            }
            memcpy(NEXT_LATCHES.MICROINSTRUCTION,next_instr,sizeof(int)*CONTROL_STORE_BITS);
            NEXT_LATCHES.STATE_NUMBER = 34;        
        }
        if(exception_caused==0 && interr==0 && rs==0){
            for(int i=0;i<CONTROL_STORE_BITS;i++){
                next_instr[i] = CONTROL_STORE[J][i];
            }
            memcpy(NEXT_LATCHES.MICROINSTRUCTION,next_instr,sizeof(int)*CONTROL_STORE_BITS);
    ...
    if(IRD==3){
        for(int i=0;i<CONTROL_STORE_BITS;i++){
                next_instr[i] = CONTROL_STORE[return_state][i];
            }
            memcpy(NEXT_LATCHES.MICROINSTRUCTION,next_instr,sizeof(int)*CONTROL_STORE_BITS);
            NEXT_LATCHES.STATE_NUMBER = return_state;  
    }
    if(PSR_changed==0){
        if(NEXT_LATCHES.Z==1){
    ...
    int save_MDR = GetSAVEMDR(CURRENT_LATCHES.MICROINSTRUCTION);
    if(save_MDR==1){
        protect_mdr= Low16bits(CURRENT_LATCHES.MDR);
    }
    int load_MDR = GetLOADMDR(CURRENT_LATCHES.MICROINSTRUCTION);
    if(load_MDR==1){
        NEXT_LATCHES.MDR = Low16bits(protect_mdr);
    }
}