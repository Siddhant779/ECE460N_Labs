...
#define MASKER(pos, wid,instr) ((instr) >> (pos) & ~(~0<<(wid)))
...
int target_pc,trap_pc,
    mem_reg_id,v_mem_ld_cc, v_mem_ld_reg,
    mem_pcmux;

/*signals for the AGEX stage*/
...
int ii,jj = 0;
  ...
  int r_w = Get_DCACHE_RW(PS.MEM_CS);
  int mem_address_zero = MASKER(0,1,PS.MEM_ADDRESS);
  int we0 = ((size) || ((size==0 && mem_address_zero==0))) && r_w==1;
  int we1 = ((size) || ((size==0 && mem_address_zero==1))) && r_w==1;
  //for en
  int dcache_en = Get_DCACHE_EN(PS.MEM_CS);
  int mem_v = PS.MEM_V;
  int v_dcache_en = dcache_en & mem_v;
  //for logic2 data
  int mem_alu_result = PS.MEM_ALU_RESULT;
  int data=0;
  if(size==0){
    data = (mem_alu_result & 0x00FF) + ((mem_alu_result << 8) & 0xFF00);
  }
  else{
    data = mem_alu_result;
  }
  ...
  else if(v_dcache_en==0){
    read_word=0;
  }
  mem_stall = v_dcache_en & !(dcache_r);
  // logic for writing into SR.DATA
  int temp_SR_DATA=0;
  if(size==0){
    if(mem_address_zero==0){
      int input = (read_word) & 0x00FF;
      temp_SR_DATA = SIGN_EXT(input,8);
    }
    else if(mem_address_zero==1){
      int input = ((read_word)>>8) & 0x00FF;
      temp_SR_DATA = SIGN_EXT(input,8);
    }
  }
  else if(size==1){
    temp_SR_DATA=(read_word);
  }
  temp_SR_DATA = temp_SR_DATA & 0xFFFF;

  //br logic 
  int n_instr = MASKER(11,1,PS.MEM_IR);
  int z_instr = MASKER(10,1,PS.MEM_IR);
  int p_instr = MASKER(9,1,PS.MEM_IR);
  int n_condition = MASKER(2,1,PS.MEM_CC);
  int z_condition = MASKER(1,1,PS.MEM_CC);
  int p_condition = MASKER(0,1,PS.MEM_CC);
  ...
  if(mem_v){
    if(br_op){
      if(((n_instr & n_condition) ||(z_instr & z_condition) || (p_instr & p_condition))){
        mem_pcmux=1;
      }
      if((n_instr==0 && z_instr==0 && p_instr==0)){
      mem_pcmux=1;
      }
    }
    else if(uncon_op){
  ...
  //logic 6 
  v_mem_ld_cc = Get_MEM_LD_CC(PS.MEM_CS) & mem_v;
  v_mem_ld_reg = Get_MEM_LD_REG(PS.MEM_CS) & mem_v;
  v_mem_br_stall = Get_MEM_BR_STALL(PS.MEM_CS) & mem_v;
  ...
  //load the trap pc
  trap_pc= temp_SR_DATA & 0xFFFF;
  NEW_PS.SR_DATA = temp_SR_DATA & 0xFFFF;
  //load the sr address 
  ...
  //load drid
  NEW_PS.SR_DRID = PS.MEM_DRID & 0x0007;
  mem_reg_id = PS.MEM_DRID & 0x0007;
...
void AGEX_stage() {
  ...
  int addr1mux_signal = Get_ADDR1MUX(PS.AGEX_CS);
  int addr2mux_signal = Get_ADDR2MUX(PS.AGEX_CS);
  int addressmux_signal = Get_ADDRESSMUX(PS.AGEX_CS);
  int sr2mux_signal = Get_SR2MUX(PS.AGEX_CS);
  int lshf1_signal = Get_LSHF1(PS.AGEX_CS);
  int aluk_signal = Get_ALUK(PS.AGEX_CS);
  int alu_resultmux_signal = Get_ALU_RESULTMUX(PS.AGEX_CS);
  int am4 = MASKER(0,4,PS.AGEX_IR);
  int off5 = MASKER(0,5,PS.AGEX_IR);
  int off6 = MASKER(0,6,PS.AGEX_IR);
  int off8 = MASKER(0,8,PS.AGEX_IR);
  int off9 = MASKER(0,9,PS.AGEX_IR);
  int off11 = MASKER(0,11,PS.AGEX_IR);
  int addr1mux_output=0;
  int addr2mux_output=0;
  int addrmux_output=0;
  if(addr1mux_signal==0){
    addr1mux_output = PS.AGEX_NPC;
  }
  else if(addr1mux_signal==1){
    addr1mux_output = PS.AGEX_SR1;
  }
  if(addr2mux_signal==0){
    addr2mux_output = 0;
  }
  else if(addr2mux_signal==1){
    addr2mux_output = SIGN_EXT(off6,6);
  }
  else if(addr2mux_signal==2){
    addr2mux_output = SIGN_EXT(off9,9);
  }
  else if(addr2mux_signal==3){
    addr2mux_output = SIGN_EXT(off11,11);
  } 
  if(lshf1_signal){
    addr2mux_output=addr2mux_output<<1;
  }
  //addr1muxoutput now has the value of 1 for addressmux
  addr1mux_output = addr1mux_output+addr2mux_output;
  int temp_addressmux_input = (off8 & 0x00FF)<<1;
  if(addressmux_signal==0){
    addrmux_output = temp_addressmux_input & 0xFFFF;
  }
  else if(addressmux_signal==1){
    addrmux_output = addr1mux_output & 0xFFFF;
  }
  int shf_sr1_output=0;
  int shift = MASKER(4,2,PS.AGEX_IR);
  if(shift & 0x0001){
    if(shift & 0x0002){
      shf_sr1_output = (SIGN_EXT(PS.AGEX_SR1,16) >>am4);
    }
    else{
      shf_sr1_output = (PS.AGEX_SR1)>>am4;
    }
  }
  else{
    shf_sr1_output = (PS.AGEX_SR1)<<am4;
  }
  shf_sr1_output = shf_sr1_output & 0xFFFF;
  int sr2mux_output=0;
  if(sr2mux_signal==0){
    sr2mux_output = PS.AGEX_SR2;
  }
  if(sr2mux_signal==1){
    sr2mux_output = SIGN_EXT(off5,5);
  }
  int alu_result=0;
  if(aluk_signal==0){
    alu_result = sr2mux_output+(PS.AGEX_SR1);
  }
  if(aluk_signal==1){
    alu_result = sr2mux_output & (PS.AGEX_SR1);
  }
  ...
  if(aluk_signal==3){
    alu_result = sr2mux_output;
  }
  int alu_result_mux_output=0;
  if(alu_resultmux_signal==0){
    alu_result_mux_output = shf_sr1_output & 0xFFFF;
  }
  else if(alu_resultmux_signal==1){
    alu_result_mux_output = alu_result & 0xFFFF;
  }
  ...
  if (LD_MEM) {
    ...
    NEW_PS.MEM_ADDRESS = addrmux_output & 0xFFFF;
    NEW_PS.MEM_NPC = PS.AGEX_NPC & 0xFFFF;
    NEW_PS.MEM_CC = PS.AGEX_CC & 0x0007;
    NEW_PS.MEM_ALU_RESULT = alu_result_mux_output & 0xFFFF;
    NEW_PS.MEM_IR = PS.AGEX_IR & 0xFFFF;
    NEW_PS.MEM_DRID = PS.AGEX_DRID & 0x0007;
...
/************************* DE_stage() *************************/
void DE_stage() {
 int CONTROL_STORE_ADDRESS = ((PS.DE_IR & 0xF800) >> 10) + ((PS.DE_IR & 0x0020) >> 5);  /* You need to implement the logic to
  ...
  int sr1 = MASKER(6,3,PS.DE_IR);
  int sr2 = ((PS.DE_IR & 0x2000) ? MASKER(9,3,PS.DE_IR): MASKER(0,3,PS.DE_IR));
  ...
  if(PS.DE_V){
    if(Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])){
      if((v_agex_ld_reg==1 && (PS.AGEX_DRID==sr1) ) || (v_mem_ld_reg && (PS.MEM_DRID==sr1)) || (v_sr_ld_reg && (PS.SR_DRID==sr1))){
        dep_stall=1;
      }
    }
      if(Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])){
         if((v_agex_ld_reg==1 && (PS.AGEX_DRID==sr2) ) || (v_mem_ld_reg && (PS.MEM_DRID==sr2)) || (v_sr_ld_reg && (PS.SR_DRID==sr2))){
          dep_stall=1;
         }
      }
  ...
  int temp_agex_v =PS.DE_V & !dep_stall;
  int dr_mux = Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
    // write the code for loading into register here 
  if(LD_AGEX) {
    /* Your code for latching into AGEX latches goes here */
    NEW_PS.AGEX_NPC = PS.DE_NPC;
    NEW_PS.AGEX_IR = PS.DE_IR;
    NEW_PS.AGEX_SR1 = sr1_data & 0xFFFF;
    NEW_PS.AGEX_SR2 = sr2_data & 0xFFFF;
    NEW_PS.AGEX_CC=agex_cc & 0x0007;
    NEW_PS.AGEX_V = temp_agex_v;
    if(dr_mux==0){
      NEW_PS.AGEX_DRID = MASKER(9,3,PS.DE_IR);
    }
    else if(dr_mux==1){
      NEW_PS.AGEX_DRID = 7;
    }
...
/* your code for FETCH stage goes here */
  int mem_pc_mux_output;
  if(mem_pcmux==0){
    mem_pc_mux_output=PC+2;
  }
  else if(mem_pcmux==1){
    mem_pc_mux_output=target_pc;
  }
  else if(mem_pcmux==2){
    mem_pc_mux_output=trap_pc;
  }
  int pc_incremented = PC+2;
  int ir;
  icache_access(PC,&ir,&icache_r);
  int LD_PC = ((!(v_de_br_stall || v_agex_br_stall || v_mem_br_stall || mem_stall || dep_stall) && icache_r) ||(v_mem_br_stall && mem_pcmux==1) || (v_mem_br_stall && mem_pcmux==2));
  if(LD_PC){
    PC=mem_pc_mux_output;
  }
  int LD_DE = !(dep_stall || mem_stall);
  if(LD_DE){
    NEW_PS.DE_NPC = pc_incremented;
    NEW_PS.DE_IR = ir;
